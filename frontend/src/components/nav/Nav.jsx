                ...
                <div className={`relative group flex items-center justify-center ${searchbar ? "block " : "hidden"} `}>
                    <input type="text" id="search"
                        className={`border-[1px] my-3 bg-transparent w-[90%]  placeholder-gray-800 outline-none  border-black px-6 py-3 rounded-3xl
                            `}
                        onChange={(e) => { e.target.value ? sethandleplaceholder(e.target.value) : sethandleplaceholder(false) }}
                    />
                    <p className={`absolute bottom-6 max-[425px]:left-10 left-16  text-xl duration-300  group-focus-within:translate-y-[-1rem] group-focus-within:text-xs group-focus-within:opacity-60 group-focus-within:duration-300 ${handleplaceholder ? "translate-y-[-1rem] duration-300 opacity-60 text-xs " : ""}`}>
                        Search
                    </p>
